...
{
    [Authorize]
    [Route("Recruiters/[controller]/[action]")]
    public class AccountJobRecruitersController : Controller
    {
        ...
        public AccountJobRecruitersController(
            UserManager<ApplicationUser> userManager,
            SignInManager<ApplicationUser> signInManager,
            IEmailSender emailSender,
            ILogger<AccountJobRecruitersController> logger,
        ...
        public async Task<IActionResult> Login(LoginViewModel model, string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                var user = await _userManager.FindByEmailAsync("IRecruiterUserI" + model.Email);
                if (user != null)
                {
                    var CheckCompanyActive = _context.AppJobRecruitersCompany.Where(c => c.CompanyIdd == user.CompanyIdd)
                                            .Select(d => d.CompanyActiveStatus).FirstOrDefault();
                    if (!await _userManager.IsEmailConfirmedAsync(user))
                    {
                        ModelState.AddModelError(string.Empty, "Vui lòng xác nhận email của bạn.");
                        return View(model);
                    }
                    else if (CheckCompanyActive == false)
                    {
                        ModelState.AddModelError(string.Empty, "Công ty của bạn đã bị khóa.");
                        return View(model);
                    }
                }
                else
                ...
                var result = await _signInManager.PasswordSignInAsync("IRecruiterUserI" + model.Email, model.Password, model.RememberMe, lockoutOnFailure: true);
        ...
        public async Task<IActionResult> Logout()
        {
            ...
            return RedirectToAction(nameof(HomeJobRecruitersController.Index), "HomeJobRecruiters");
        }
